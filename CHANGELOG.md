...
- Per-push CI moved from GitHub Actions to CircleCI to cut GitHub Actions
  minute spend. The seven jobs in `.github/workflows/test.yml` (removed) are
  consolidated into three CircleCI jobs in `.circleci/config.yml`:
  `tests-and-validation` (BATS, frontmatter, Codex parity, security scan,
  JSON/YAML/TOML validation), `routing-evals`, and `documentation-build`.
  Fewer containers per push also keeps CircleCI credit spend down; all jobs
  run on the `small` resource class.
- Still on GitHub Actions deliberately: `docs.yml` (GitHub Pages OIDC deploy
  only works from Actions, and it is path-filtered to docs changes),
...
- `skills/delivery-record-verify/` — new read-only skill that validates a Delivery Record against the schema (resolving the version from `predicate_type`), enforces the per-`activity_type` required-keys policy, and applies the threshold rule (a `fail` needs a `## Waiver`; an `n/a` needs a one-line justification) as soft warnings or, with `--strict`, hard failures. Includes an `agents/openai.yaml` Codex policy.
- `scripts/delivery_record_verify.py` + `scripts/delivery-record-verify.sh` — the shared Python validator (stdlib + `pyyaml` + `jsonschema`) and a CI wrapper. The wrapper is a drop-in CI lint for other projects (`scripts/delivery-record-verify.sh --strict`); it is **not** enabled in cms-cultivator's own CI by default.
- `docs/spec/delivery-record/v1.md` (the predicate-URL target), `docs/commands/delivery-record.md`, and `docs/commands/delivery-record-verify.md` doc pages, with new `Specification` and Skills nav entries in `zensical.toml`.
- `tests/test-delivery-record.bats` and `tests/test-delivery-record-verify.bats` — spec-validates-against-itself, bad-records-fail, per-activity required-key, schema/sub-schema sync, `--strict`, and auto-detect coverage.
- `README.md`, `skills/README.md`, and `docs/agents-and-skills.md` updated for the two new skills.

## [1.4.0] - 2026-06-19

### Added
- `skills/wp-plugin-to-private-package/` — new skill that converts a committed (or hand-installed) WordPress premium plugin into a Kanopi private Composer package and rewires the consuming site to install it via Composer. Implements Kanopi's WordPress Core and Plugins Installation Policy (§3 paid plugins, §4 Kanopi Private Packagist): Phase 0 discovery (rules out WPackagist/vendor endpoints and existing packages), Phase 1 private repo creation (composer.json, topics, team write access, version release tag), Phase 2 consuming-repo rewire (require + Satis repository entry, `.gitignore` exception removal, `git rm --cached`, docs), and Phase 3 Satis indexing + `composer update` + commit. `disable-model-invocation: true` with an `agents/openai.yaml` Codex policy (explicit confirmation required). `tests/test-plugin.bats` and `skills/README.md` updated accordingly.
- `skills/wp-devops-setup/` — new skill that onboards an existing Pantheon WordPress site to Kanopi's DevOps system: the Composer-managed `kanopi/wp-pantheon-starter` layout with DDEV, CircleCI → Pantheon deploys (multidev on PRs, dev on `main`), PHPCS/Rector/PHPStan gates, Lighthouse/pa11y/BackstopJS, Quicksilver hooks (search-replace, WP-CFM, New Relic, post-deploy), and scheduled "update dev" / "automatic updates" pipelines. The WordPress counterpart to the Drupal-focused `devops-setup`. A dedicated Phase 5 delegates premium/committed-plugin packaging to the `wp-plugin-to-private-package` skill. `disable-model-invocation: true` with an `agents/openai.yaml` Codex policy. Distilled from a real green-innovation-index Pantheon WordPress onboarding. New `docs/commands/wp-devops-setup.md` + nav entry; `tests/test-plugin.bats`, `skills/README.md`, `docs/commands/overview.md`, and `docs/agents-and-skills.md` updated accordingly.
